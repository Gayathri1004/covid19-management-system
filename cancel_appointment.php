...
<body>
  ...
  <div id="page-container">
    <div class="content">
      <div id="content-wrap">
      <h3 class="center-align">Cancellation Confirmation</h3><br><br>
      <?php 
					include('dbconnection.php');
					

					$sql = " SELECT * FROM doc_details WHERE book_id = '".$book_id."'  ";
					...
					if($count>=1){
						...
						print "<p align='center'>Sorry, there are no appointments to be cancelled!!!</p>";
					}
                    $con->close();
					?>
...
<div class="row">
            ...
            <center>
              <button class="btn waves-effect waves-light black" type="submit" name="submit" value="submit">CANCEL<i class="material-icons right">send</i></button>
            </center>
            </div>
          </div>
</form>
...
$sql = "DELETE FROM doc_details where book_id='".$book_id."'";
$result=$con->query($sql);
if ($result) {
echo "<script>alert('Your booking has been Cancelled!');</script>";    
echo '<script>location.href="view_appointment.php"</script>';
} else {
echo "<script>alert('There was an Error in cancellation')<script>";
}
$con->close();
}
?>
