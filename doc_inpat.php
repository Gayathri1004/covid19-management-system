...
<body>
  ...
  <div id="page-container">
    <div class="content">
      <div id="content-wrap">
      <h3 class="center-align">View Total In-Patients</h3><br><br>
  ...
  <div id="admin" class="col s12">
    <div class="card material-table">
      <?php
        extract($_POST);
        if(isset($save)) {
            $sql="select * from patient_reg where (date_in between '$from' and '$to') OR (date_out between '$from' and '$to') OR (date_in <= '$from' and date_out >= '$to') ";
            $qsql = mysqli_query($con,$sql); 
            $row_count=mysqli_num_rows($qsql);
            if ($qsql->num_rows > 0) {
                echo "<table id='datatable'><thead><tr><th><b>Patient Name</b></th><th><b>Gender</b></th><th><b>Age</b></th><th><b>City</b></th><th><b>Admission Date</b></th><th><p><b>Discharge Date</b></th></tr></thead>";
                // output data of each row
                while($row = $qsql->fetch_assoc()) {
                  echo "<tbody><tr><td>" . $row["name"]. "</td><td>"  . $row["gender"]. "</td><td>"  . $row["age"]. "</td><td>". $row["city"]. "</td><td> " . $row["date_in"].  "</td><td> " . $row["date_out"].  "</td></tr></tbody>";
                }
                echo "</table>";
              } else 
              {
